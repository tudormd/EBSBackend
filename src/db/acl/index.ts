...
class CasbinAcl {
                ...
                switch (group.name) {
                    case 'admin': {
                        for (const user of group.users) {
                            ...
                            await e.addGroupingPolicy(user.id.toString(), user.id + '_' + group.name);
                        };
                        break;
                    }
                    case 'manager': {
                        for (const user of group.users) {
                            await e.addPolicy(user.id + '_' + group.name, '/api/group' + '/' + group.id, 'GET');
                            await e.addGroupingPolicy(user.id.toString(), user.id + '_' + group.name);
                        };
                    }
                }
    ...
    private async getAdapter() {
        return await SequelizeAdapter.newAdapter('postgresql://' +
            ...
            '/' +
            (process.env.DATABASE_DB || 'postgresql_dev'), true);
    }
